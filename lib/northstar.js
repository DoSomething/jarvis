...
module.exports = {
   ...
   */
  put(path, token, data) {
    console.log('Making northstar PUT request', path);

    const authToken = `Bearer ${token}`;
    return new Promise((resolve) => {
      superagent.put(`${uri}${path}`)
        .send(data)
        .set(authHeader, authToken)
        .then(res => resolve(res.body))
        .catch(error => resolve({ error }));
  ...
  createUser(userData) {
    return this.auth()
    .then(token => this.post('/v1/users', token, userData))
    .catch(err => console.error(err));
  },
  ...
  findUser(type, value) {
    return this.auth()
    .then(token => this.get(`/v1/users/${type}/${value}`, token))
    .catch(err => console.error(err));
  },
  ...
  /**
   * Update a user profile with the given fields
   * @param {String} northstarId
   * @param {Object} fields
   *
   */
  updateUser(northstarId, fields) {
    return this.auth()
    .then(token => this.put(`/v1/users/_id/${northstarId}`, token, fields))
    .catch(err => console.error(err));
  },
};
